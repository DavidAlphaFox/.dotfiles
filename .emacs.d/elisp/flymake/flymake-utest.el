...
(defun flymaketest-make-output-buffer(&optional name)
    ...
    (let ((buffer (get-buffer-create name))
          (output-file-name (flymake-create-temp-inplace "flymaketest" "flymake")))
        (set-buffer buffer)
...
(defun test-flymake-save-source-buffer-name()
    (flymake-reg-names 123 "saved-buf")
...
(defun test-flymake-parse-err-lines()
    (let* ((test-data '(
		   ...
		   (count (length test-data)))	
		(flymake-set-buffer-data (current-buffer) (makehash 'equal))
	    (while (> count 0)
		    (let* ((before            (nth 0 (nth (1- count) test-data)))
				   (lines             (nth 1 (nth (1- count) test-data)))
				   (master-file-name  (nth 2 (nth (1- count) test-data)))
				   (patched-master-file-name (flymake-create-temp-inplace (if master-file-name master-file-name "") "flymake_master"))
				   (source-file-name  (nth 3 (nth (1- count) test-data)))
				   (patched-source-file-name (flymake-create-temp-inplace (if source-file-name source-file-name "") "flymake"))
				   (base-dir          (nth 4 (nth (1- count) test-data)))
				   (after             (nth 5 (nth (1- count) test-data))))

                (flymake-log 0 "testing %d" (1- count))

                (flymake-set-buffer-value (current-buffer) "base-dir"              base-dir)
				(flymake-set-buffer-value (current-buffer) "master-file-name"      master-file-name)
				(flymake-set-buffer-value (current-buffer) "temp-master-file-name" patched-master-file-name)
				(flymake-set-buffer-value (current-buffer) "temp-source-file-name" patched-source-file-name)

			    (assert-equal after (flymake-parse-err-lines before (current-buffer) lines))
			)
		    (setq count (1- count))
		)
    )
...
(defun test-flymake-get-real-name()
    (let* ((test-data'(
	    ...
	    (while (> count 0)
			...
			(flymake-set-buffer-value (current-buffer) "base-dir" (nth 1 (nth (1- count) test-data)))
			(flymake-set-buffer-value (current-buffer) "temp-source-file-name" "d:/projects/a_flymake.cpp")
			(flymake-set-buffer-value (current-buffer) "master-file-name" "d:/projects/a.cpp")
			(flymake-set-buffer-value (current-buffer) "temp-master-file-name" "d:/projects/a_flymake_master.cpp")
			
		    (assert-equal (nth 2 (nth (1- count) test-data))
						  (flymake-get-real-file-name (current-buffer)
...
(defun test-flymake-get-common-path-prefix()
    (assert-equal nil (flymake-get-common-path-prefix "abc" "abcde"))
    (assert-equal "a/"   (flymake-get-common-path-prefix "a" "a"))
    (assert-equal nil   (flymake-get-common-path-prefix "cba" "abc"))
)
...
(defun test-flymake-build-relative-path()
    (let* ((test-data  '( ("d:/projects/a" "d:/projects/b" "../b/")
						  ...
						  ("d:/a/b/c/b"    "d:/e"          "../../../../e/")
						  ("/usr/lib/ext"  "/usr/lib"      "../")
						  ("/usr/lib"      "/usr2/lib"     "../../usr2/lib/")
						  ("/usr/lib"      "/usr/lib/ext"  "ext/")
						  ))
		   (count      (length test-data)))
	    (while (> count 0)
		    (assert-equal (nth 2 (nth (1- count) test-data))
...
(defun flymake-test-suite()
    ...
    (test-flymake-create-temp-inplace)
	...
	(test-flymake-get-common-path-prefix)
	(test-flymake-build-relative-path)
...
(load "D:/Projects/flymake/flymake.el")
(eval-buffer nil nil)
